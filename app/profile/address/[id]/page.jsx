      ...
      if (res.status === 200) {
        ...
        setTambons(
          provinces
            ...
            ?.districts.filter(
              (p) => `อ.${p.name_th}` === res?.data?.district
            )[0]?.sub_districts
        );
      }
    } catch (error) {
  ...
  return (
    <div className="w-full">
      <Link
        ...
        className="p-2.5 px-3 w-fit text-sm text-white bg-blue-500 flex items-center gap-2 rounded-md shadow-md"
      >
        <FaArrowLeft />
        <p>กลับ</p>
      </Link>{" "}
      <div className="flex mt-3 flex-col pb-3 w-full border-b border-gray-300">
        <p className=" ">
          {Number(id) === 0 ? "เพิ่มที่อยู่ใหม่" : "แก้ไขข้อมูลที่อยู่"}
        </p>
      </div>
      {/* address */}
      ...
      {/* phone */}
      <span className="flex items-center mt-8 gap-3.5 lg:w-[60%] w-full pl-3">
        <p className="text-sm text-gray-600 w-[20%]">เบอร์โทรศัพท์</p>
        <div className="flex flex-col w-[75%]">
          <Controller
            name="phone"
            rules={{
              ...
              validate: (value) => {
                if (!isValidThaiPhone(value)) return "เบอร์โทรศัพท์ไม่ถูกต้อง";
              },
            }}
            control={control}
            render={({ field }) => (
      ...
      <div className="mt-8 w-full flex flex-col items-start">
        <span className="flex items-center gap-3.5 lg:w-[60%] w-full pl-3">
          <p className="text-sm text-gray-600 w-[20%]">สถานะ</p>
